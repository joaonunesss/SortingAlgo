...
/*   Updated: 2023/07/27 17:24:54 by jmarinho         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "push_swap.h"

int	min_max(t_list *stack_b, t_info_a info_a)
{
	t_info_b	info_b;
	int			target;

	target = max(stack_b);
	info_b.size_b = ft_lstsize(stack_b);
	info_b.rotations_b = check_position(stack_b, target);
	return (mv_calcs(info_a, info_b));
}

int	btw_min_max(t_list *stack_b, int content_a, t_info_a info_a)
{
	t_list		*temp_b;
	t_info_b	info_b;
	int			target;
...
t_nbr_info	b_target(t_list *stack_b, int content_a)
{
	t_list		*temp_b;
	t_nbr_info	target;
